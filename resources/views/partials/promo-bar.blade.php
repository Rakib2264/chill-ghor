...
@if ($promoEnabled)
    <div
        class="bg-gradient-warm text-white text-center py-2.5 px-4 text-sm font-semibold flex items-center justify-center gap-3">
        <span>{{ $promoText }}</span>
        <a href="{{ $promoLink }}"
            class="underline underline-offset-2 font-bold hover:opacity-80 transition">{{ $promoButtonText }}</a>
    </div>
@endif
