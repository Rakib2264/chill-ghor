...
<head>
  ...
  <style>
    body { font-family: 'Hind Siliguri', sans-serif; background: #f0ece6; color: #2a1d18; }
    .font-display { font-family: 'Tiro Bangla', serif; }
    .gradient-text { background: linear-gradient(135deg,#c0392b,#e8671a); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
...
<body class="min-h-screen" x-data="{ sidebar: false }">
  ...
  {{-- ===== SIDEBAR ===== --}}
  <aside class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full transform flex-col border-r border-white/10 transition-transform duration-300 lg:translate-x-0"
         style="background: linear-gradient(175deg, #1a0d0a 0%, #2a1d18 50%, #3d2d28 100%);"
         :class="sidebar && '!translate-x-0'">

    {{-- Brand --}}
    <div class="flex h-16 items-center gap-3 border-b border-white/10 px-5">
      <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl bg-gradient-warm shadow-warm text-lg">
    ...
    <div class="border-t border-white/10 p-3">
      <div class="flex items-center gap-3 rounded-xl bg-white/7 px-3 py-2.5">
        <div class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-warm text-sm font-bold text-white">
          {{ strtoupper(substr(auth()->user()->name ?? 'A', 0, 1)) }}
        </div>
        <div class="min-w-0 flex-1">
          <div class="truncate text-sm font-semibold text-[#f5ddd5]">{{ auth()->user()->name ?? 'Admin' }}</div>
  ...
  <div class="lg:pl-64">
    ...
    {{-- Topbar --}}
    <header class="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-charcoal/10 bg-white/85 px-4 backdrop-blur-md sm:px-6">
    ...
    <main class="p-4 sm:p-6 lg:p-8">
      ...
      {{-- Toast --}}
      @if (session('toast'))
        <div x-data="{ s: true }" x-init="setTimeout(() => s = false, 3500)"
             x-show="s" x-transition.opacity x-cloak
