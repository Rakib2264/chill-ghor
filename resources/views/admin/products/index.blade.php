...
<div class="rounded-2xl border border-charcoal/10 bg-white shadow-soft overflow-hidden">
  ...
  <div class="flex flex-wrap items-center gap-3 border-b border-charcoal/8 p-4">
    <form method="GET" class="flex flex-1 flex-wrap gap-2 min-w-[260px]">
      ...
      <button class="rounded-full bg-charcoal px-5 py-2 text-sm font-bold text-cream hover:bg-charcoal/85 transition">
        খুঁজুন
      </button>
    </form>

    <a href="{{ route('admin.products.trash') }}"
       class="rounded-full bg-cream px-4 py-2 text-sm font-bold">
      🗑️ ট্র্যাশ ({{ $trashCount ?? 0 }})
    </a>
    ...
    <table class="w-full text-sm">
      <thead>
        <tr class="bg-cream/80 text-xs uppercase tracking-wider text-charcoal/50">
          ...
          <th class="py-3 px-4 text-center">স্টক</th>
          <th class="py-3 px-4 text-center">ভিউ</th>
      ...
      <tbody>
        @forelse($products as $p)
          <tr class="border-t border-charcoal/5 hover:bg-cream/40 transition-colors">
            ...
            <td class="py-3 px-5">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-charcoal/45 truncate max-w-[180px]">{{ $p->description }}</div>
                </div>
              </div>
            </td>
            ...
            <td class="px-4 py-3 text-right">
              ...
              @if($p->old_price)
                <div class="text-[11px] text-charcoal/35 line-through">৳{{ number_format($p->old_price) }}</div>
              @endif
            </td>
            ...
            <td class="px-4 py-3 text-center text-base space-x-0.5">
              ...
              @if($p->show_on_home) <span title="হোমে আছে">🏠</span> @endif
            </td>

            {{-- ✅ Stock --}}
            <td class="px-4 py-3 text-center">
              @if($p->stock === -1)
                <span class="text-base font-black text-green-600" title="সীমাহীন">∞</span>
              @elseif($p->stock === 0)
                <span class="rounded-full bg-red-100 px-2.5 py-1 text-xs font-black text-red-700">শেষ</span>
              @elseif($p->stock <= 5)
                <span class="rounded-full bg-orange-100 px-2.5 py-1 text-xs font-black text-orange-700">⚡ {{ $p->stock }}টি</span>
              @else
                <span class="text-xs font-bold text-charcoal/60">{{ $p->stock }}টি</span>
              @endif
            </td>

            {{-- ✅ Views --}}
            <td class="px-4 py-3 text-center">
              <span class="text-xs text-charcoal/50">👁 {{ $p->views_label }}</span>
            </td>
            ...
            <td class="px-4 py-3 text-center">
              <form action="{{ route('admin.products.toggle-status', $p->id) }}" method="POST"
                ...
                <button type="submit"
                        class="rounded-full px-2.5 py-1 text-[10px] font-bold uppercase transition hover:scale-105
            ...
            <td class="py-3 px-5 text-right">
              <div class="inline-flex items-center gap-1.5">
                ...
                <form action="{{ route('admin.products.destroy', $p) }}" method="POST"
                  ...
                  @csrf @method('DELETE')
                  <button class="rounded-lg border border-red-200 bg-white px-3 py-1.5 text-xs font-bold text-red-500 transition hover:bg-red-50">
                    🗑️
                  </button>
                </form>
              </div>
        ...
        @empty
          <tr>
            <td colspan="8" class="py-16 text-center text-charcoal/40">
