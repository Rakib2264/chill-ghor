  ...
  <div class="mt-10 grid gap-6 lg:grid-cols-2">
    ...
    <div class="space-y-4">
      @foreach ($contactInfo as $info)
        <div class="flex items-center gap-4 rounded-2xl border border-charcoal/10 bg-white p-5 shadow-soft">
          <div class="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl bg-primary/10 text-xl">{{ $info[0] ?? '📍' }}</div>
          <div>
            <div class="text-xs font-bold text-charcoal/50 uppercase tracking-wider mb-0.5">{{ $info[1] ?? '' }}</div>
            @if(($info[1] ?? '') == 'ফোন')
              <a href="tel:{{ preg_replace('/[^0-9+]/', '', $info[2] ?? '') }}" class="font-bold text-sm hover:text-primary transition">{{ $info[2] ?? '' }}</a>
            @elseif(($info[1] ?? '') == 'ইমেইল')
              <a href="mailto:{{ $info[2] ?? '' }}" class="font-bold text-sm hover:text-primary transition">{{ $info[2] ?? '' }}</a>
            @else
    ...
    <div class="rounded-2xl border border-charcoal/10 bg-white p-7 shadow-soft">
      ...
      <form method="POST" action="{{ route('contact.store') }}" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-charcoal/50 mb-1.5">{{ $nameLabel }} <span class="text-primary">*</span></label>
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-charcoal/50 mb-1.5">{{ $emailLabel }} <span class="text-primary">*</span></label>
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-charcoal/50 mb-1.5">ফোন (ঐচ্ছিক)</label>
          <input type="tel" name="phone" value="{{ old('phone') }}" placeholder="০১XXXXXXXXX"
                 class="w-full rounded-xl border border-charcoal/15 bg-cream px-4 py-2.5 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/15">
        </div>
        
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-charcoal/50 mb-1.5">{{ $messageLabel }} <span class="text-primary">*</span></label>
