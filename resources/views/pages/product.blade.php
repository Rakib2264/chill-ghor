...
<div class="min-h-screen" style="background:#f8f4f0;">
  ...
  <div class="mx-auto max-w-6xl px-4 pt-5 sm:px-6 lg:px-8">
    <nav class="flex items-center gap-1.5 text-xs text-charcoal/40 font-medium">
      ...
      <svg class="h-3 w-3 text-charcoal/25" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    ...
    <div class="grid gap-6 lg:grid-cols-[55%_1fr] lg:gap-10 lg:items-start">
      ...
      <div class="relative">
          ...
          <div class="absolute top-4 left-4 right-4 flex items-center justify-between">
            ...
            @if ($product->popular)
              <div class="flex items-center gap-1.5 rounded-full px-3.5 py-1.5 shadow-lg backdrop-blur-sm"
                ...
                <span class="text-[11px] font-black uppercase tracking-wider text-[#1c0f09]">জনপ্রিয়</span>
              </div>
            @else
              <div></div>
            @endif
            ...
            <form action="{{ route('wishlist.toggle', $product) }}" method="POST">
              ...
              <button type="submit"
                      class="group flex items-center gap-2 rounded-full px-4 py-2.5
                             shadow-lg backdrop-blur-md border transition-all duration-200
                             hover:scale-105 active:scale-95 font-bold text-xs
                             {{ $inWish
                                ? 'bg-red-500 text-white border-red-400'
                                : 'bg-white/92 text-charcoal/55 border-white/60 hover:bg-red-500 hover:text-white hover:border-red-400' }}">
          ...
          <div class="absolute bottom-4 left-4 right-4 flex items-end justify-between">
            <div class="rounded-2xl px-4 py-2.5 backdrop-blur-md shadow-lg"
              ...
              <div class="flex items-baseline gap-2">
                <span class="font-display text-2xl font-black text-primary">৳{{ number_format($product->price) }}</span>
                @if ($product->old_price)
                  <span class="text-sm text-charcoal/35 line-through">৳{{ number_format($product->old_price) }}</span>
                @endif
              </div>
              @if ($product->old_price)
                <div class="text-[10px] font-bold text-green-600 mt-0.5">
                  {{ round((($product->old_price - $product->price) / $product->old_price) * 100) }}% ছাড় পাচ্ছেন!
                </div>
              @endif
            </div>
        ...
        @if ($product->reviews_count > 0)
          ...
          <div class="mt-3 flex items-center justify-between rounded-2xl bg-white px-5 py-3 shadow-sm border border-charcoal/6">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-[10px] text-charcoal/40 mt-0.5">{{ $product->reviews_count }} গ্রাহক রিভিউ</p>
              </div>
            </div>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="rounded-[28px] bg-white border border-charcoal/6 shadow-sm p-6">
          ...
          <div class="hidden lg:flex items-baseline gap-3">
            <span class="font-display text-4xl font-black text-primary">৳{{ number_format($product->price) }}</span>
            @if ($product->old_price)
              <span class="text-lg text-charcoal/30 line-through">৳{{ number_format($product->old_price) }}</span>
              <span class="rounded-full bg-green-50 px-2.5 py-1 text-xs font-black text-green-600 border border-green-100">
        ...
        <div class="grid grid-cols-3 gap-2">
          @foreach([
            ['🚚','দ্রুত','ডেলিভারি','rgba(192,57,43,0.08)'],
            ['💵','ক্যাশ','অন ডেলিভারি','rgba(245,166,35,0.10)'],
            ['✅','তাজা','উপাদান','rgba(22,163,74,0.08)'],
          ] as [$icon,$title,$sub,$bg])
            <div class="flex flex-col items-center gap-2 rounded-2xl bg-white border border-charcoal/6 py-4 shadow-sm">
              <div class="flex h-10 w-10 items-center justify-center rounded-xl" style="background:{{ $bg }}">
  ...
  <div class="mx-auto max-w-6xl px-4 pb-10 sm:px-6 lg:px-8">
    <div class="rounded-[32px] bg-white border border-charcoal/6 shadow-sm overflow-hidden">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-charcoal/8 px-6 py-5">
        <div>
          ...
          <h2 class="font-display text-xl font-black text-[#1c0f09] sm:text-2xl">
            গ্রাহকদের মতামত
            <span class="ml-1.5 rounded-full bg-primary/10 px-2.5 py-0.5 text-sm font-bold text-primary">{{ $product->reviews_count }}</span>
          </h2>
        </div>
        @if ($product->reviews_count > 0)
          @php $avg = $product->average_rating; @endphp
          <div class="flex items-center gap-3 rounded-2xl px-4 py-2.5" style="background:#f8f4f0;">
            ...
            <div>
              <div class="flex gap-0.5">
                @for ($i = 1; $i <= 5; $i++)
                  <svg class="h-3.5 w-3.5 {{ $i <= round($avg) ? 'text-[#f5a623]' : 'text-charcoal/15' }}" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                  </svg>
                @endfor
              </div>
      ...
      <div class="p-6 space-y-4">
            ...
            <div class="mb-4 flex items-center gap-1">
              @for ($i = 1; $i <= 5; $i++)
                <button type="button" @mouseenter="hover={{ $i }}" @mouseleave="hover=0" @click="rating={{ $i }}"
                        ...
                        :class="(hover || rating) >= {{ $i }} ? 'text-[#f5a623]' : 'text-charcoal/15'">★</button>
              @endfor
              <input type="hidden" name="rating" :value="rating">
              <span class="ml-2 rounded-full bg-white px-2.5 py-0.5 text-sm font-black shadow-sm" x-text="rating + '/5'"></span>
            </div>
        ...
        @else
          <div class="rounded-2xl border border-dashed border-charcoal/15 px-5 py-6 text-center" style="background:#f8f4f0;">
            <p class="text-sm text-charcoal/50">রিভিউ দিতে চাইলে <a href="{{ route('login') }}" class="font-bold text-primary hover:underline">লগইন করুন</a></p>
          </div>
        @endauth

        <div class="space-y-3">
          @forelse ($product->reviews as $rev)
            <div class="rounded-2xl border border-charcoal/7 bg-white p-4 sm:p-5 transition hover:shadow-sm">
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="text-sm font-black text-[#1c0f09]">{{ $rev->user->name ?? 'Guest' }}</div>
                    <div class="mt-0.5 flex items-center gap-2">
                      <div class="flex gap-0.5">
                        @for ($i = 1; $i <= 5; $i++)
                          <svg class="h-3 w-3 {{ $i <= $rev->rating ? 'text-[#f5a623]' : 'text-charcoal/15' }}" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                          </svg>
                        @endfor
                      </div>
                      <span class="text-[11px] text-charcoal/35">{{ $rev->created_at->diffForHumans() }}</span>
                    </div>
                  </div>
                </div>
                @auth
                  @if ($rev->user_id === auth()->id() || (auth()->user()->is_admin ?? false))
                    <form action="{{ route('reviews.destroy', $rev) }}" method="POST" onsubmit="return confirm('মুছে ফেলবেন?')">
                      @csrf @method('DELETE')
                      <button class="flex h-7 w-7 items-center justify-center rounded-full text-charcoal/20 hover:bg-red-50 hover:text-red-500 transition text-sm">✕</button>
                    </form>
                  @endif
                @endauth
              </div>
          ...
          @empty
            <div class="rounded-2xl border border-dashed border-charcoal/12 py-10 text-center text-sm text-charcoal/40" style="background:#f8f4f0;">
